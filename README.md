<h1 align="center">Student Voice-학생의 목소리</h1>

<p align="center">
	<a href="https://github.com/Quackk08/studentvoice/releases"><img src="https://img.shields.io/badge/version-v1.4--stable-blue?style=flat-square" /></a>
	<a href="https://github.com/Quackk08/studentvoice/releases"><img src="https://img.shields.io/badge/deployment-2nd%20release-success?style=flat-square" /></a>
	<a href="https://github.com/Quackk08/studentvoice/issues"><img src="https://img.shields.io/github/issues-raw/Quackk08/studentvoice.svg?style=flat-square&label=issues" /></a>
	<a href="https://github.com/Quackk08/studentvoice/blob/main/LICENSE"><img src="https://img.shields.io/badge/license-MIT-green?style=flat-square" /></a>
</p>

-------

### Description:
Lightweight web application designed to collect, manage, and track student opinions and feedback in real-time.

The platform provides a transparent communication channel between students and school administrations (or student councils). It allows users to submit suggestions anonymously or publicly, vote on ongoing agendas, and check the status of active requests through a streamlined dashboard.

To run the project in development mode, create a ".env" file in the root directory with your application keys.

### Technical Stack & Environment:
- **Frontend / Backend:** Next.js (React), Node.js
- **Styling / UI:** Tailwind CSS, Radix UI
- **Deployment:** Vercel (2nd Official Release)

### Quackk Quackk 🦆
